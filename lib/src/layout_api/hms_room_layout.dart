...
class HMSRoomLayout {
  ...
  static Future<void> getRoomLayout({
    ...
    debugPrint('[HMSRoomLayout] getRoomLayout called');
    debugPrint('[HMSRoomLayout] authToken length: ${authToken.length}');
    debugPrint('[HMSRoomLayout] endPoint: $endPoint');
    debugPrint('[HMSRoomLayout] roleName: $roleName');
    ...
    debugPrint('[HMSRoomLayout] Response type: ${value?.runtimeType}');
    if (value is String) {
      debugPrint('[HMSRoomLayout] Response preview: ${value.substring(0, value.length > 200 ? 200 : value.length)}...');
    } else if (value is HMSException) {
      debugPrint('[HMSRoomLayout] HMSException: code=${value.code}, message=${value.message}, description=${value.description}');
    }
    
    if (value != null && value.runtimeType != HMSException) {
      debugPrint('[HMSRoomLayout] Parsing layout JSON...');
      _setLayout(layoutJson: jsonDecode(value));
      resetLayout(roleName);
      debugPrint('[HMSRoomLayout] Layout set successfully');
    } else {
      debugPrint('[HMSRoomLayout] Layout fetch failed or returned exception');
    }
  }
