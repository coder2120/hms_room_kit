...
class Utilities {
  ...
  ///This function gets permissions for the camera and microphone.
  ///
  /// Spedia Uni fork: upstream also required Bluetooth (iOS) and phone (Android).
  /// Basic video lessons work with speaker/earpiece; Bluetooth is optional for headsets.
  static Future<bool> getPermissions() async {
    ...
    final camOk = cam.isGranted || cam.isLimited;
    ...
    final isCameraPermissionsDenied = (cam.isDenied &&
        !await Permission.camera.shouldShowRequestRationale);
    final isMicrophonePermissionsDenied = (mic.isDenied &&
        !await Permission.microphone.shouldShowRequestRationale);

    if (isCameraPermissionsDenied || isMicrophonePermissionsDenied) {
  ...
  /// Spedia Uni fork: camera + microphone only (no Bluetooth / phone gate).
  ///
  /// Always calls [Permission.request] first: on some OS/plugin versions the
  /// status stays `denied` until the app has gone through the request channel,
  /// even when Settings already show Camera/Mic enabled.
