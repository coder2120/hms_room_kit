...
class _ScreenControllerState extends State<ScreenController> {
  ...
  Future<HMSException?> _getAuthTokenAndSetLayout(
    ...
    debugPrint('[ScreenController] _getAuthTokenAndSetLayout called');
    debugPrint('[ScreenController] roomCode: ${Constant.roomCode}');
    debugPrint('[ScreenController] authToken: ${Constant.authToken != null ? "present (${Constant.authToken!.length} chars)" : "null"}');
    
    if (Constant.roomCode != null) {
      debugPrint('[ScreenController] Getting auth token by room code...');
      ...
      debugPrint('[ScreenController] Got tokenData: ${tokenData?.runtimeType}');
    } else {
      debugPrint('[ScreenController] Using provided authToken directly');
      tokenData = Constant.authToken;
    }

    if ((tokenData is String?) && tokenData != null) {
      debugPrint('[ScreenController] Token is valid string, fetching room layout...');
      debugPrint('[ScreenController] layoutAPIEndPoint: ${Constant.layoutAPIEndPoint}');
      try {
        ...
        debugPrint('[ScreenController] Room layout fetched successfully');
      } catch (e) {
        debugPrint('[ScreenController] Error fetching room layout: $e');
        rethrow;
      }
      return null;
    } else {
      debugPrint('[ScreenController] Token is not a valid string: ${tokenData?.runtimeType}');
      return tokenData;
    }
  }
  ...
  void _initPreview() async {
    debugPrint('[ScreenController] _initPreview started');
    Constant.roomCode = widget.roomCode;
    if (mounted) {
      setState(() {
        isLoading = true;
    ...
    debugPrint('[ScreenController] Building HMSSDKInteractor...');
    ...
    debugPrint('[ScreenController] HMSSDKInteractor built successfully');

    debugPrint('[ScreenController] Getting auth token and layout...');
    HMSException? ans;
    try {
      ans = await _getAuthTokenAndSetLayout(
        _hmsSDKInteractor,
    ...
    if (ans != null && mounted) {
      ...
      debugPrint('[ScreenController] Auth/layout succeeded, setting isLoading=false');
